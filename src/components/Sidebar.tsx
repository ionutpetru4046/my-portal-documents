...
export default function Sidebar({ open, setOpen }: { open?: boolean; setOpen?: (v: boolean) => void }) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      const emailAllowed = allowlist.includes(email.toLowerCase());
  ...
  const MenuLinks = () => (
    <nav className="flex-1 flex flex-col mt-4">
      <Link href="/dashboard" className="px-6 py-3 hover:bg-indigo-100 hover:text-indigo-700 transition rounded-lg mx-2 my-1" onClick={closeMenu}>
        Dashboard
      </Link>
      <Link href="/profile" className="px-6 py-3 hover:bg-indigo-100 hover:text-indigo-700 transition rounded-lg mx-2 my-1" onClick={closeMenu}>
        Profile
      </Link>
      {isAdmin && (
        <div className="mt-4 pt-4 border-t border-gray-200 mx-4">
          ...
          <Link href="/admin/users" className="px-6 py-3 hover:bg-indigo-100 hover:text-indigo-700 transition rounded-lg mx-2 my-1" onClick={closeMenu}>
            Users
          </Link>
          <Link href="/admin/expirations" className="px-6 py-3 hover:bg-indigo-100 hover:text-indigo-700 transition rounded-lg mx-2 my-1" onClick={closeMenu}>
            Expirations
          </Link>
        </div>
      )}

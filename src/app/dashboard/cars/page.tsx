...
export default function CarsPage() {
  ...
  const handleUpload = async () => {
    ...
    const filePath = `cars-documents/${fileName}`;
    ...
    setFilteredDocs((prev) => [newDoc, ...prev]);
  };
    ...
    <main className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-10">
      ...
      <div className="max-w-7xl mx-auto text-center mb-10">
        ...
        <p className="mt-2 text-gray-500 sm:text-lg md:text-xl">
          Upload, view, and manage all car-related documents
        </p>
      </div>

      {/* Upload Section */}
      <div className="max-w-3xl mx-auto mb-10 p-6 bg-white rounded-3xl shadow-md flex flex-col sm:flex-row gap-4 items-center">
        <input
          type="file"
          onChange={(e) => setFile(e.target.files?.[0] || null)}
          className="border p-3 rounded-xl w-full sm:flex-1"
          disabled={uploading}
        />
        <Button
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
          ...
          <div className="relative w-full max-w-5xl h-[80vh]">
            <Image
              src={encodeURI(filteredDocs[currentIndex].url)}

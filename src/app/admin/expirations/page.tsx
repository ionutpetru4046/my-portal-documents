...
function badge(expiration_date: string | null) {
  ...
  if (days < 0) return <span className="px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-600">Expired</span>;
  if (days <= 7) return <span className="px-2 py-1 rounded-full text-xs bg-red-100 text-red-700">Expiring in {days}d</span>;
  return null;
}
...
export default function AdminExpirationsPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      let req = supabase.from("documents").select("id,name,userID,expiration_date,reminder_at");
      if (filterUserID) req = req.eq("userID", filterUserID);
  ...
  const filtered = useMemo(() => {
    ...
    let arr = docs.filter(d => !q || d.name.toLowerCase().includes(q));
    arr = arr.sort((a, b) => {

...
import { FiEdit, FiTrash2, FiCheckCircle, FiFileText, FiActivity, FiBell, FiUpload, FiTrendingUp, FiCalendar, FiClock, FiAlertCircle, FiX, FiLogOut } from "react-icons/fi";
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="relative">
            <button
              ...
              className="relative p-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 transition-colors duration-200 border border-slate-700"
            >
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-96 bg-slate-900 rounded-xl shadow-2xl border border-slate-800 z-50 overflow-hidden">
                <div className="p-4 border-b border-slate-800 flex items-center justify-between">
                  <h3 className="font-semibold text-white">Notifications</h3>
                  <button onClick={() => setShowNotifications(false)} className="text-slate-400 hover:text-slate-300">
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifList.length > 0 ? (
                      ...
                      <div key={n.id + n.type} className="p-4 border-b border-slate-800 hover:bg-slate-800/50 transition-colors last:border-b-0">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-white text-sm">{n.name}</p>
                            <p className="text-xs text-slate-400 mt-1">{n.info}</p>
                          </div>
                        </div>
                      </div>
                    ))
                    ...
                    <div className="p-8 text-center">
                      <FiBell className="w-12 h-12 mx-auto mb-2 text-slate-700" />
          ...
          <Reveal animation="fade-up">
            <div className="lg:col-span-1">
                  ...
                  <div className="flex flex-col items-center -mt-16 mb-6">
                    <div className="relative group mb-4">
                      <img
                        ...
                        alt="Avatar"
                        className="w-28 h-28 rounded-full border-4 border-slate-900 shadow-lg object-cover"
                      />
                      <button
                    ...
                    <p className="text-slate-400 text-sm text-center mt-1">{user.email}</p>
                  </div>

                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-300 mb-2 uppercase tracking-wide">Full Name</label>
                      <input
                        type="text"
                        name="name"
                        value={formData.name}
                        ...
                        className={`w-full px-3 py-2.5 rounded-lg border transition-all text-sm ${
                          editMode 
                            ? "border-blue-500 bg-slate-800 text-white focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20" 
                    ...
                    <div>
                      ...
                      <input
                        ...
                        disabled
                        className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-800/50 text-slate-300 text-sm"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 mt-6">
                    {editMode ? (
                      <>
                        ...
                        <Button 
                          className="px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm transition-colors"
              ...
              <div className="mt-6 space-y-2">
                <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg bg-slate-800 hover:bg-slate-700 transition-colors text-slate-300 text-sm font-medium">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Reveal animation="fade-up" delay={40}>
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-slate-900 rounded-lg border border-slate-800 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-white mt-1">{docCount}</h3>
                    </div>
                ...
                <div className="bg-slate-900 rounded-lg border border-slate-800 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">Alerts</p>
                      <h3 className="text-2xl font-bold text-white mt-1">{notifCount}</h3>
                    </div>
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                      notifCount > 0 ? 'bg-red-500/20 text-red-400' : 'bg-slate-700 text-slate-400'
                    }`}>
                ...
                <div className="bg-slate-900 rounded-lg border border-slate-800 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">Recent</p>
                      <h3 className="text-2xl font-bold text-white mt-1">{recentDocs.length}</h3>
                    </div>
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-slate-400 mt-0.5">Added {formatDate(doc.created_at)}</p>
                            </div>
                          </div>
                          {doc.expiration_date && (
            ...
            <Reveal animation="fade-up" delay={120}>
              <div className="bg-slate-900 rounded-lg border border-red-900/50 overflow-hidden">
                <div className="px-6 py-4 border-b border-red-900/50 bg-red-500/10">
                  <h3 className="font-semibold text-red-400 flex items-center text-sm">
